...
class DataDownloader:
    """
    Orchestrates downloading a single JSON endpoint task.

    Supports all OPSDataType values (biblio, abstract, claims, etc.) — the
    data type is baked into the DownloadTask and reflected in the URL.

    Responsibilities:
    - skip policy (already downloaded)
    - rate limiting per attempt
    - retry loop via RetryPolicy
    - cross-thread backoff via rate_limiter.apply_penalty() on 429
    ...
    def fetch_and_store(self, download_task: DownloadTask, *, session: requests.Session) -> DownloadResult:
        """
        Download one task and persist the result. Returns a DownloadResult
        regardless of success, skip, or failure so the caller can log uniformly.
        """
        ...
        for attempt_number in range(1, self._retry_policy.max_attempts + 1):
            ...
            try:
                ...
                decision = self._retry_policy.on_request_exception(
                    exc=exc, attempt_number=attempt_number
                )
                last_message = f"{decision.reason}; sleeping {decision.sleep_seconds:.1f}s"
